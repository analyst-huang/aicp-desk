...
> 本项目不是金山云官方 SDK。控制台接口升级后，部分字段可能需要同步更新。创建、启动、停止和保存镜像都会真实影响云端资源。

## 功能

- 开发机：查询、创建、启动、停止、保存镜像、复制公网 SSH 命令、存为模板。
- 训练任务：查询、创建、启动、停止、存为模板。
...
开发机列表提供独立的“启动”和“停止”按钮。运行中的开发机还可以：
...
点击“新建训练任务”后，可配置训练专用资源组/队列、镜像、GPU、挂载、运行命令、运行时长、自愈与保留策略。列表提供独立的启动、停止和存为模板按钮。
