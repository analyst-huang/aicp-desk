...
该选项只输出本地说明，不需要登录或访问云端。说明采用“实验周期授权”：用户一次批准实验目标、允许操作的代码/数据范围、总资源或费用硬上限和停止条件后，Agent 可以在边界内自主调参、调试、创建、启动、停止、重启和清理本周期任务。每次提交前仍需执行 `--dry-run`；周期已获授权且操作没有越界时，Agent 可以自行使用 `--yes`，不需要逐次询问。
