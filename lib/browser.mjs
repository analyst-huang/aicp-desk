...
import { appPaths, ensureAppDirs, exists } from "./paths.mjs";
...
export class BrowserSession {
  ...
  browserArgs({ headless = false, url = this.config.consoleUrl } = {}) {
    const args = [
  ...
  async launchLogin() {
    ...
    const child = spawn(edge, this.browserArgs({ headless: false, url: loginUrl }), {
      detached: true,
      stdio: "ignore",
      windowsHide: false,
    });
  ...
  async launchHeadless() {
    ...
    const edge = await findEdge(this.config);
    const child = spawn(edge, this.browserArgs({ headless: true }), {
      detached: false,
      stdio: "ignore",
      windowsHide: true,
    });
