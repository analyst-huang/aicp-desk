...
export class BrowserSession {
  ...
  browserArgs({ headless = false, url = this.config.consoleUrl, passwordStore } = {}) {
    const args = [
  ...
  async launchLogin({ display, passwordStore } = {}) {
    ...
    const browserEnvironment = display ? { ...process.env, DISPLAY: display } : process.env;
    if (display) delete browserEnvironment.WAYLAND_DISPLAY;
    const child = spawn(edge, this.browserArgs({ headless: false, url: loginUrl, passwordStore: effectivePasswordStore }), {
      detached: true,
      stdio: "ignore",
      windowsHide: false,
  ...
  async launchHeadless() {
    ...
    const child = spawn(edge, this.browserArgs({ headless: true, passwordStore }), {
      detached: false,
      stdio: "ignore",
      windowsHide: true,
    });
