...
export async function installRemoteUiRuntime({ allowNoSandbox = false, allowRoot = false, platform = process.platform } = {}) {
...
export async function detectContainerEnvironment({ env = process.env, existsFn = exists, readFileFn = readFile } = {}) {
  ...
  try {
    ...
    return /(?:docker|kubepods|containerd|libpod|podman|lxc)/i.test(cgroup);
  } catch {
    return false;
  }
}
...
export async function remoteUiDoctor(config, rawOptions = {}, overrides = {}) {
  ...
  return {
    ...
    privateRuntime: {
      ...
      mode: runtimeMode,
      strategy: runtimeManifest.scope === "aicp-hybrid-runtime" ? "environment-first" : "private-only",
      sources: {
