...
export async function remoteUiStatus({ paths = appPaths(), platform = process.platform } = {}) {
  ...
  const accessStopped = Boolean(state.accessStopped && processStatus.xvfb);
  return {
    configured: true,
    running,
    accessStopped,
    sessionHostRunning: Boolean(processStatus.xvfb),
...
export async function suspendRemoteUi({ paths = appPaths(), platform = process.platform } = {}) {
  const state = await readJson(paths.remoteUiState, null);
  if (!state) return { stopped: true, wasRunning: false, accessStopped: true, sessionKept: false, statePath: paths.remoteUiState };
  const xvfb = (state.processes ?? []).find((record) => record.name === "xvfb");
  if (!xvfb || !(await managedProcessAlive(xvfb, platform))) {
    return { ...(await stopRemoteUi({ paths, platform })), accessStopped: true, sessionKept: false };
  }
  let stopped = false;
  for (const record of [...(state.processes ?? [])].reverse()) {
    if (record.name === "xvfb") continue;
  ...
  const suspendedState = {
    ...state,
    accessStopped: true,
    accessStoppedAt: new Date().toISOString(),
    processes: [xvfb],
  };
  await writeJsonAtomic(paths.remoteUiState, suspendedState);
  return {
    stopped: true,
    wasRunning: stopped,
    accessStopped: true,
...
export async function startRemoteUi(config, rawOptions = {}, overrides = {}) {
  ...
  const sameSettings = existing.display === options.display
    && existing.vncPort === options.vncPort
    && existing.webPort === options.webPort;
  if (existing.running) {
    if (sameSettings) {
  ...
  if (existing.configured && !resuming) await stopRemoteUi({ paths, platform });
  ...
  let specs = remoteUiProcessSpecs(options, {
    ...doctor.dependencies,
    noVnc: doctor.dependencies.noVnc,
  }).map((spec) => ({
    ...spec,
    env: { ...spec.env, ...desktopEnvironment },
    logPath: path.join(paths.home, `remote-ui-${spec.name}.log`),
  }));
  const processes = [];
  if (resuming) {
    const suspendedState = await readJson(paths.remoteUiState, null);
    const xvfb = (suspendedState?.processes ?? []).find((record) => record.name === "xvfb");
    if (!xvfb || !(await managedProcessAlive(xvfb, platform))) {
      await stopRemoteUi({ paths, platform });
  ...
  try {
    for (const spec of specs) {
      const record = await spawnManaged(spec);
    ...
    for (const record of startedProcesses.reverse()) await terminateRecord(record, { platform });
    throw error;
  }
}
