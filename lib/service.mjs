  ...
  async gpuCapacity(options = {}) {
    ...
    const pools = (response.groups ?? []).map(({ pool, gpu, queues, nodes }) => {
      ...
      const normalizedNodes = (nodes ?? []).map((node) => {
        ...
        right.remainingGpu - left.remainingGpu
    ...
    return {
      ...
      summary: {
        ...
        nodeCount: pools.reduce((sum, pool) => sum + pool.nodes.length, 0),
        gpuNodeCount: pools.reduce((sum, pool) => sum + pool.nodes.filter((node) => node.isGpu).length, 0),
      },
      pools,
    };
  }
