...
export class AicpService {
  ...
  async gpuCapacity(options = {}) {
    ...
    const pools = (response.groups ?? []).map(({ pool, gpu, queues, nodes }) => {
      ...
      const freeGpu = availableValue === null || availableValue === undefined
      ...
      const normalizedQueues = (queues ?? []).map((queue) => {
        ...
        return {
          ...
          remainingGpu: hasGpuQuota ? Math.max(0, quotaGpu - allocatedGpu) : null,
      ...
      return {
        ...
        visibleNodeCount: normalizedNodes.length,
      };
    });
    return {
      ...
      summary: {
        ...
        freeGpu: pools.reduce((sum, pool) => sum + pool.freeGpu, 0),
        ...
        visibleNodeCount: pools.reduce((sum, pool) => sum + pool.visibleNodeCount, 0),
      },
  ...
  async trainingLogs(selector, options = {}) {
    ...
    const response = await this.api.trainJobPods(item.TrainJobId, {
      region: query.region,
    ...
    for (const pod of selected) {
      const result = await this.api.trainJobLog(item.TrainJobId, pod.Name, query);
      logs.push({
        pod,
