  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if (request.method === "POST" && routeMatch(requestUrl.pathname, "/api/dev/action")) {
        const body = await readBody(request);
        if (!["start", "stop"].includes(body.action)) throw new Error("开发机 action 必须是 start 或 stop");
      ...
      if (request.method === "POST" && routeMatch(requestUrl.pathname, "/api/train/action")) {
        const body = await readBody(request);
        if (!["start", "stop"].includes(body.action)) throw new Error("训练任务 action 必须是 start 或 stop");
